...
defmodule RandomForest do
  ...
  def build_forest(dataset, criteria_list, data_sample_size, crit_number, tree_number) do
    1..tree_number |> Task.async_stream(fn _ ->
    ...
    end, max_concurrency: 30, timeout: :infinity) |> Enum.map(fn {:ok, v} -> v end)
  end
  ...
  def find_value(forest, record, needed_accuracy \\ 0.5) do
    vote = forest |> Enum.map(fn tree ->
      {:ok, r} = DecisionTree.find_value(tree, record)
      r
    end)
    # Logger.debug("#{inspect vote}")
    if Enum.count(vote, & &1 == :yes) > needed_accuracy * Enum.count(vote), do: :yes, else: :no
  end
